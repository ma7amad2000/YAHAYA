#⚡MOHAMED HELAL⚡
import random
from YukkiMusic.utils.decorators.language import (LanguageStart,LanguageStartCB)
...
@app.on_callback_query(filters.regex("devplay"))
@LanguageStartCB
async def dev_panel(client, CallbackQuery: CallbackQuery,_):
    await CallbackQuery.answer()
    callback_data = CallbackQuery.data.strip()
    callback_request = callback_data.split(None, 1)[1]
    videoid, chat_id = callback_request.split("|")
       ...
       reply_markup=InlineKeyboardMarkup(
          [
               [
                    ...
                    InlineKeyboardButton(
                        text=_["S_B_6"], user_id=Dev),
                    InlineKeyboardButton(
                        text=_["S_B_11"], user_id=DevYowu_Id),
                ],[
                    InlineKeyboardButton(
